...
interface Props {
  ...
  initialEvents: NewsPost[];
  initialTab: (typeof TABS)[number];
}

export function NewsPageClient({ initialPosts, initialEvents, initialTab }: Props) {
  ...
  const { data: posts } = useQuery({
    queryKey: queryKeys.news(tab),
    queryFn: () => api.get<NewsPost[]>(`/news${tab === "ALL" ? "" : `?kind=${tab}`}`),
    initialData: tab === initialTab ? initialPosts : undefined,
    staleTime: 60_000,
  });

  const { data: events } = useQuery({
    queryKey: queryKeys.eventsUpcoming(),
    ...
    <div className="container-page py-8">
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-[1fr_300px]">
        <div className="space-y-4">
              ...
              <article className="card flex gap-4 p-4 transition hover:shadow-md hover:border-gold/30">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {p.sourceName && <span className="text-xs text-ink/30">· {p.sourceName}</span>}
                  </div>
          ...
          {(posts ?? []).length === 0 && <p className="text-sm text-ink/40">ยังไม่มีข่าวสาร</p>}
        </div>
        ...
        <aside className="card h-fit p-5">
          ...
          <button type="button" onClick={() => setTab("EVENT")} className="btn-primary mt-4 w-full">
            VIEW ALL EVENTS
          </button>
        </aside>
      </div>
